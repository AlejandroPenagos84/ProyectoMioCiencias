...
namespace Arbol
{
    ...
    public:
        ...
        Nodo<T, S> *createNodo(T Data, S Valor);
    ...
    template <class T, class S>
    Nodo<T, S> *RBTree<T, S>::createNodo(T Data, S Valor)
    {
    ...
    Queue<Nodo<T, S> *> RBTree<T, S>::recorridoNiveles()
    {
        ...
        while (!cola.IsEmpty())
        {
            Nodo<T, S> *nodo = cola.Front('I');
            cola.Dequeue('I');
            resultado.Enqueue(nodo, 'I');
            if (nodo->left && nodo->left->Clave != T())
            {
                cola.Enqueue(nodo->left, 'I');
            }
            if (nodo->right && nodo->right->Clave != T())
            {
                cola.Enqueue(nodo->right, 'I');
            }
        }
        return resultado;
    }
